...
//=============================================================================
 /*:
 * @plugindesc (aria version 2) v1.15 Adds more features to the Message Window to customized
...
Window_Base.prototype.isCJKCharacter = function(char) {
    if (!char) return false;
    var code = char.charCodeAt(0);
    return (code >= 0x3000 && code <= 0x9FFF) ||  // CJK Symbols, Hiragana, Katakana, CJK Unified Ideographs
...
Window_Base.prototype.isKinsokuChar = function(char) {
    if (!char) return false;
    var kinsoku = '。、.,!?！？」』）】〉》〕｝・：；ー～っゃゅょぁぃぅぇぉァィゥェォッャュョ%‰';
    return kinsoku.indexOf(char) >= 0;
};
...
Window_Base.prototype.checkWordWrap = function(textState) {
    ...
    var char = textState.text[textState.index];
    var nextChar = textState.text[textState.index + 1];

    if (this.isCJKCharacter(char)) {
        var charWidth = this.textWidthExCheck(char);

        if (textState.x + charWidth > this.contents.width) {
            return true;
        }

        if (nextChar && this.isKinsokuChar(nextChar)) {
            var combinedWidth = charWidth + this.textWidthExCheck(nextChar);
            if (textState.x + combinedWidth > this.contents.width) {
                return true;
            }
        }
        return false;
